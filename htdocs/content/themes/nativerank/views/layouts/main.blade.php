    ...
    <header id="masthead" class="site-header">
        <div class="site-branding">
            ...
            @if(is_front_page() && is_home())
                <h1 class="site-title"><a href="{{ esc_url(home_url('/')) }}" rel="home">{{ get_bloginfo('name') }}</a>
                </h1>
            @else
                <p class="site-title"><a href="{{ esc_url( home_url('/')) }}" rel="home">{{ get_bloginfo('name') }}</a>
                </p>
            @endif
        ...
        <nav id="site-navigation" class="main-navigation">
            <button class="menu-toggle" aria-controls="primary-menu"
                    aria-expanded="false">{{ esc_html__('Primary Menu', THEME_TD) }}</button>
            {!! wp_nav_menu([
