...
class AssetServiceProvider extends ServiceProvider
{
    ...
    public function register()
    {
        ...
        if (App::environment('local')) {
            $version = Date::now()->getTimestamp();

        }

        Asset::add('theme_styles', 'css/theme.css', [], $version)->to('front');
//        Asset::add('theme_woo', 'css/woocommerce.css', ['theme_styles'], $theme->getHeader('version'))->to('front');
        Asset::add('theme_js', 'js/theme.min.js', [], $version)->setType('script')->attributes(['defer'])->to('front');
    }
}
